...
all_dialogues <- map_df(json_pfade, function(datei) {
  ...
  df <- imap(raw_list, function(scene_block, scene_index) {
    ...
    map_df(scene_block, function(x) {
      ...
      # only using dialog
      if (!is.na(speaker) && !is.null(txt) && txt != "") {
        return(tibble(
          scene_id = scene_index,
          character = as.character(speaker), 
...
# preprocessing
data("stop_words")
namen_filter <- c( "continue", "contd", "vo", "dont", "hes", "its", "im", "thats")
...
# export whole dataset as a csv file into the folder results
write_csv(scorsese_master, "results/scorsese_cleaned_master.csv")
...
# export overview into a csv file also into the results folder
write_csv(inventur, "results/scorsese_film_summary.csv")